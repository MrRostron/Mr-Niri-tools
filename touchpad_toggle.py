...
def open_config(path: Path) -> tuple[list[str], int | None]:
    """
    Opens and reads the niri config file config.kdl
    This function returns our configuration file as a tuple containing
    a list containing each line in the configuration and an int
    our line number that our inline comment toggleOffTouchpad is located at.
...
def write_config(path: Path, config: list) -> None:
    """
    Write Config function rewrites our configuration file setting
    either toucpad off or // off
...
def toggle_on_off(line_num: int, contents: list) -> list:
    """
    Toggle function sets the touchpad setting in the niri configuration file
    to off or // off while keeping users indentation.
